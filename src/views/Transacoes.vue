    ...
    <template v-slot:body="{ items }">
      <tbody>
        <tr v-for="(item, index) in items" :key="index">
          <td v-bind:class="{ transacaoInativa: !item.ativo }">
            {{ item.empresa }}
          </td>
          <td v-bind:class="{ transacaoInativa: !item.ativo }">
            {{ item.cliente.nome }}
          ...
          <td>
            <v-icon
              ...
              class="mr-2"
              @click="editItem(item)"
              v-bind:title="messageVisuzalizar"
              >mdi-magnify</v-icon
            >
            <v-icon
              v-if="item.ativo"
              size="20"
              @click="ativarDesativarTransacao(item)"
        ...
        <v-dialog v-model="dialog" max-width="500px" overlay-color="grey">
          <template v-slot:activator="{ on }">
            <v-btn color="primary" dark v-on="on">
              <v-icon dark v-on="on" v-bind:title="messageNewAdd"
                >mdi-plus</v-icon
              >
            </v-btn>
          </template>
              ...
              <v-form ref="form" v-model="valid">
                <v-container>
                  <v-row>
                    ...
                    <v-col cols="12" sm="6" md="4">
                      <v-text-field
                        v-model="editedItem.cliente.nome"
                        :readonly="readonly"
                        :rules="clienteRules"
                        :counter="25"
                        label="Cliente"
...
export default {
  ...
  data: (vm) => ({
    ...
    status: ["Em Andamento", "Cancelado", "Concluído"],
    transacoes: [],
    editedIndex: -1,
    ...
    valorRules: [(v) => !!v || "Valor Obrigatório"],
  }),
  ...
  methods: {
    ...
    balancoTotal() {
      ...
      this.totalGerado = parseFloat(
        (this.valorTotalTransacoes * 0.01).toFixed(2)
      );
    },
    ...
    editItem(item) {
      this.dialog = true;
      this.readonly = true;
      if (!item.ativo) this.readstatus = true;
    ...
    ativarDesativarTransacao(item) {
      ...
      else resp = confirm("Tem certeza que deseja reativar essa transação?");
    ...
    async save() {
      if (
        ...
        //Object.assign(this.transacoes[this.editedIndex], this.editedItem)
          ...
          await TransacaoRequestUtil.salvar(this.editedItem).then(
            (editedItem) => {
